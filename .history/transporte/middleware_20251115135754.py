...
class PermisosMiddleware:
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        # ✅ DIAGNÓSTICO - AGREGAR ESTOS PRINTS
        print(f"🔍 URL actual: {request.resolver_match.url_name}")
        ...
        if request.user.is_authenticated and (request.user.is_superuser or request.user.is_staff):
            print("🔍 Usuario es superuser/staff - Acceso permitido")
            return None

        # Solo aplicar a usuarios autenticados
        if not request.user.is_authenticated:
            print("🔍 Usuario no autenticado - Continuar")
            return None
        ...
        if request.resolver_match.url_name in urls_publicas:
            print(f"🔍 URL pública - Acceso permitido: {request.resolver_match.url_name}")
            return None
        ...
        if not url_actual:
            print("🔍 No hay URL name - Acceso permitido")
            return None
            
        # Verificar si la URL actual está permitida para este usuario
        if url_actual not in permisos.get(tipo_usuario, []):
            print(f"🚫 ACCESO DENEGADO: Usuario {tipo_usuario} no puede acceder a {url_actual}")
            print(f"🚫 Permisos disponibles para {tipo_usuario}: {permisos.get(tipo_usuario, [])}")
            messages.error(request, '❌ No tiene permiso para acceder a esta función')
            return redirect('dashboard')

        print(f"✅ ACCESO PERMITIDO: Usuario {tipo_usuario} puede acceder a {url_actual}")
        return None