...
class PublicReadOnly(permissions.BasePermission):
    """
    Permiso personalizado que permite acceso de solo lectura público
    """
    def has_permission(self, request, view):
        # Permitir todas las solicitudes GET, HEAD, OPTIONS
...
class RutaViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [PublicReadOnly]  # ✅ Permite lectura pública
...
def seguros_view(request):
    seguros = Seguro.objects.all()
    return render(request, 'transporte/seguros.ht
                  ml', {'seguros': seguros})
@login_required
