...
def crear_vehiculo(request):
    ...
    if request.method == 'POST':
        try:
            # Obtener datos del formulario
            patente = request.POST.get('patente')[:10]  # ✅ Limitar a 10 caracteres
            ...
            print(f"🔍 Datos del formulario - Conductor ID: {conductor_id}")
    ...
    # ✅ CARGAR CONDUCTORES ACTIVOS CORRECTAMENTE
    conductores = Conductor.objects.filter(activo=True)
    print(f"🔍 Conductores disponibles: {conductores.count()}")
...
def crear_aeronave(request):
    ...
    if request.method == 'POST':
        try:
            # Obtener datos del formulario
            matricula = request.POST.get('matricula')[:10]  # ✅ Limitar a 10 caracteres
            ...
            print(f"🔍 Datos del formulario - Piloto ID: {piloto_id}")
    ...
    pilotos = Piloto.objects.filter(activo=True)
    print(f"🔍 Pilotos disponibles: {pilotos.count()}")
